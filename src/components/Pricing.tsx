import { ArrowUpRight, Check, Handshake, ShieldCheck, Sparkles } from "lucide-react";
    ...
    <section id="piani" className="relative overflow-hidden bg-[#070b17] px-5 py-28 text-white md:px-8 md:py-36">
      ...
      <div className="relative mx-auto max-w-7xl">
        <div className="mx-auto mb-16 max-w-3xl text-center">
          <span className="mb-5 inline-flex items-center gap-2 rounded-full border border-indigo-300/15 bg-indigo-300/[.06] px-4 py-2 text-xs font-bold uppercase tracking-[.2em] text-indigo-200"><Sparkles className="h-3.5 w-3.5" />Quattro modi per partire</span>
        ...
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
          {plans.map((plan) => (
            <article key={plan.name} className={`group relative flex min-h-[460px] flex-col overflow-hidden rounded-[2rem] border p-7 transition duration-500 hover:-translate-y-2 ${plan.featured ? "border-indigo-400/40 bg-gradient-to-b from-indigo-500/20 to-white/[.035] shadow-[0_30px_80px_-35px_rgba(99,102,241,.7)]" : "border-white/[.08] bg-white/[.035] hover:border-white/15"}`}>
              ...
              <div className="mb-8 flex h-12 w-12 items-center justify-center rounded-2xl border border-white/10 bg-white/[.06]">{plan.partnership ? <Handshake className="h-5 w-5 text-emerald-300" /> : <ArrowUpRight className="h-5 w-5 text-indigo-300" />}</div>
              ...
              <div className="my-7 border-y border-white/[.08] py-6">
                <span className="block text-xs font-bold uppercase tracking-[.16em] text-slate-500">Anticipo</span>
        ...
        <div className="mt-8 flex flex-col gap-4 rounded-2xl border border-white/[.08] bg-white/[.03] p-5 text-sm leading-relaxed text-slate-400 md:flex-row md:items-center md:p-6">
          ...
          <p><strong className="text-slate-200">Trasparenza prima di tutto.</strong> I costi operativi e di gestione sono esclusi da tutti i piani. Nel modello 50/50 li anticipiamo noi fino all'arrivo dei primi ricavi e vengono contabilizzati nel progetto. Termini, perimetro e modalità di calcolo sono definiti nel contratto dedicato.</p>
        </div>
      </div>
    </section>
  );
